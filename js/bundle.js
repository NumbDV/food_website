...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
    ...
    tabsParent.addEventListener('click', (event) => {
        ...
        if (target && target.classList.contains(tabsSelector.slice(1))) { // подставит без первого символа (точки)
            tabs.forEach((item, i) => {
                if (target == item) {
                    hideTabContent();
                    showTabContent(i);
...
window.addEventListener('DOMContentLoaded', () => {
	...
	(0,_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '2022-05-29');
});
})();
